"""Implementation of a n-dimensional Tucker factorized FNO."""
...
class TFNO(eqx.Module):
    ...
    a sequence of TFNO blocks (spectral convolutions + skip connections)
    ...
    Args:
        ...
        hidden_channels: Hidden channel dimension (latent width)
            used throughout the TFNO blocks. This significantly affects
        ...
        use_channel_mlp: Whether to apply a pointwise channel MLP
            after each TFNO block. Defaults to `True`.
        local_operator: Type of skip connection inside TFNO blocks.
        ...
        normalization: Type of normalization to use in TFNO blocks.
        ...
        use_fno_residual: Whether to use residual connection
            around TFNO blocks. Default is True.
        ...
        positional_embedding: Positional embedding to apply
            to last channels of raw input before passing through TFNO.
    ...
    !!! info
        The TFNO has the same architecture as the FNO except for the
        factorized spectral weights.

    !!! info "Upcoming Features"
        Current implementation doesn't support different TFNO block precisions,
    ...
    def __init__(
        ...
        lkey, pkey, tfno_key = jr.split(key, 3)
